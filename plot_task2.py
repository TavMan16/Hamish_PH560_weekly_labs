"""Plot saved Green's function data from Task 2 without rerunning the solver."""
...
def plot_charge_heatmap(charge_green, grid_size, start_row, start_column, total_walkers):
    ...
    plt.title(
        ...
        f"(N={grid_size}, start=({start_row}, {start_column}), walkers={total_walkers})"
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"green_charge_heatmap_N{grid_size}_r{start_row}_c{start_column}_w{total_walkers}.png",
        dpi=300,
    )
    plt.close()
...
def plot_charge_surface(charge_green, grid_size, start_row, start_column, total_walkers):
    ...
    plt.tight_layout()
    plt.savefig(
        f"green_charge_surface_N{grid_size}_r{start_row}_c{start_column}_w{total_walkers}.png",
        dpi=300,
    )
    plt.close()
...
def plot_boundary_heatmap(boundary_green, grid_size, start_row, start_column, total_walkers):
    ...
    plt.tight_layout()
    plt.savefig(
        f"green_boundary_heatmap_N{grid_size}_r{start_row}_c{start_column}_w{total_walkers}.png",
        dpi=300,
    )
    plt.close()
...
def main():
    """Load saved Green's function arrays and generate plot files."""
    ...
    charge_green = np.load(f"green_charge_{file_stub}.npy")
    boundary_green = np.load(f"green_boundary_{file_stub}.npy")

    print(f"Loaded green_charge_{file_stub}.npy")
    print(f"Loaded green_boundary_{file_stub}.npy")
